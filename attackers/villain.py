...
class Attacker(BaseVFL):
    def __init__(self, args, model, train_dataset, test_dataset):
        ...
        if self.args.num_passive - len(self.args.attack_id) == 0:
            ...
            self.rate = 0.4 * (self.args.num_passive - len(self.args.attack_id)) / len(self.args.attack_id)  # the same as VILLAIN paper
        ...
        for epoch in range(self.args.epochs):
            ...
            for batch_idx, batch_data in enumerate(self.train_dataset):
                ...
                # save the emb
                    ...
                    for attacker in self.args.attack_id:
                        # generate trigger
                        std_dev = self.std_devs[attacker]
                        trigger = []
                        trigger_size = emb[attacker][0].flatten().shape[0]
                        for i in range(trigger_size):
                            if i % 4 == 0:
                                trigger.append(std_dev)
                            elif i % 4 == 1:
                                trigger.append(std_dev)
                            elif i % 4 == 2:
                                trigger.append(-std_dev)
                                ...
                                trigger.append(-std_dev)
                        trigger = torch.tensor(trigger).reshape(emb[attacker][0].shape)
                        trigger = trigger * self.rate
                        # load the manipulated emb
                        ...
                        for i in manipulated_data_list:
                            ...
                            if self.args.defense == 'detection' and detected_flag <= self.args.detection_rate:
                                ...
                                if len(real_source_idx[0]) == 0 or j_manipulate >= len(real_source_idx[0]):
                                    emb[attacker][i] = self.cluster_centers[attacker][self.args.source_label].to(torch.float32).reshape(emb[attacker][0].shape) + trigger
                                else:
                                    source_sample_idx = real_source_idx[0][j_manipulate]
                                    j_manipulate += 1
                                    emb[attacker][i] = emb[attacker][source_sample_idx] + trigger
